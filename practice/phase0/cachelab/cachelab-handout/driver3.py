#!/usr/bin/env python3
#
# driver3.py - Python 3 版评分驱动（原 driver.py 为 Python 2，2026-09-08 由 2to3 手工迁移）
#
#   相对原版的改动只有三处，均为 Python 2 -> 3 的兼容性改动，不改任何评分逻辑：
#     1. print 语句 -> print() 函数
#     2. map(int, ...) -> list(map(int, ...))   （py3 的 map 是迭代器，不能按下标取）
#     3. subprocess 输出改为 text=True           （py3 返回 bytes，re 处理不了）
#
#   用法：python3 driver3.py
#
import subprocess
import re
import os
